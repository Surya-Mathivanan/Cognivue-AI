...
function HRDashboard() {
  ...
  useEffect(() => {
    ...
    // Fetch HR user info
    const userPromise = fetch(`${API_BASE_URL}/hr/me/`, {
      credentials: "include",
    })
      .then((r) => r.json())
      .then((data) => {
        if (data.name) setHrUser(data);
      })
      .catch(() => {});

    // Fetch available roles
    const rolesPromise = fetch(`${API_BASE_URL}/hr/roles/`, {
      credentials: "include",
    })
      .then((r) => r.json())
    ...
    Promise.all([userPromise, rolesPromise]).finally(() =>
  ...
  const handleLogout = async () => {
    setLoading(true);
    try {
      await fetch(`${API_BASE_URL}/hr/logout/`, {
        method: "POST",
        credentials: "include",
      });
    } catch (e) {
  ...
  const handleUidSearch = async (e) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/hr/search/uid/?uid=${encodeURIComponent(uidQuery.trim())}`,
        {
          credentials: "include",
        },
      );
  ...
  const handleRoleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/hr/search/role/?role=${encodeURIComponent(selectedRole)}`,
        {
          credentials: "include",
        },
      );
  ...
  const handleViewSession = async (sessionId) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/hr/session/${sessionId}/`, {
        credentials: "include",
      });
      const data = await res.json();
  ...
  if (initLoading) {
    return (
      ...
      >
        <Loader message="Loading HR dashboard..." />
      </div>
    );
  }
          ...
          <div className="hr-detail-grid">
            <div className="hr-detail-card">
              ...
              <div className="hr-info-rows">
                <div className="hr-info-row">
                  <span>Role:</span>
                  <strong>{session.role || "General"}</strong>
                </div>
                <div className="hr-info-row">
                  <span>Mode:</span>
                  <strong className="capitalize">{session.mode}</strong>
                </div>
                <div className="hr-info-row">
            ...
            <div className="hr-detail-card">
              ...
              <div className="hr-info-rows">
                <div className="hr-info-row">
                  <span>Phone:</span>
                  <strong>{candidate?.phone_number || "—"}</strong>
                </div>
                <div className="hr-info-row">
                  <span>Location:</span>
                ...
                <div className="hr-info-row">
                  <span>Degree:</span>
                  <strong>{candidate?.degree || "—"}</strong>
                </div>
                <div className="hr-info-row">
                  <span>Branch:</span>
            ...
            {session.feedback?.category_scores && (
              <div className="hr-detail-card">
                ...
                {Object.entries(session.feedback.category_scores).map(
                  ([key, val]) => (
                    <div key={key} className="hr-score-bar-row">
                      <span>{key.replace(/_/g, " ")}</span>
                      <div className="hr-score-bar-track">
                        <div
                          className="hr-score-bar-fill"
                          style={{
                            width: `${val}%`,
          ...
          <div className="hr-detail-card" style={{ marginTop: "16px" }}>
            ...
            {session.questions?.map((q, i) => (
              <div key={i} className="hr-qa-item">
                ...
                <div className="hr-qa-question">
                  <strong>Q{i + 1}:</strong> {q.text}
                </div>
                <div className="hr-qa-answer">
                  <span>A:</span>{" "}
                  {session.answers?.[i] || (
                    <em style={{ color: "#475569" }}>No answer provided</em>
                  )}
                </div>
              </div>
            ))}
    ...
    <div className="hr-dashboard">
          ...
          {hrUser && (
            <div className="hr-user-chip">
              ...
              <div>
                ...
                <div className="hr-user-company">
                  {hrUser.company || hrUser.email}
                </div>
              </div>
            </div>
          )}
      ...
      <div className="hr-content">
        <div className="hr-page-title">
          ...
          <p>
            Find candidates who have completed interviews in the Cognivue AI
            platform
          </p>
        </div>
        ...
        {error && (
          <div className="hr-error-msg" style={{ marginBottom: "16px" }}>
            ⚠️ {error}
          </div>
        )}
          ...
          <div className="hr-search-panel">
            <form onSubmit={handleUidSearch} className="hr-search-form">
              ...
              <button
                ...
                disabled={loading || !uidQuery.trim()}
              >
              ...
              <div className="hr-uid-result">
                <div className="hr-candidate-profile-card">
                  ...
                  <div className="hr-candidate-profile-info">
                    ...
                    <div className="hr-uid-badge">
                      {uidResult.candidate?.uid}
                    </div>
                    <p>
                      <strong>Email:</strong> {uidResult.candidate?.email}
                    </p>
                    <p>
                      <strong>Phone:</strong>{" "}
                      {uidResult.candidate?.phone_number || "—"}
                    </p>
                    <p>
                      <strong>Location:</strong>{" "}
                      {uidResult.candidate?.location || "—"}
                    </p>
                    <p>
                      <strong>College:</strong>{" "}
                      {uidResult.candidate?.college_name || "—"}
                    </p>
                    <p>
                      <strong>Skills:</strong>{" "}
                      {uidResult.candidate?.skills || "—"}
                    </p>
                    <p style={{ color: "#10b981", marginTop: "8px" }}>
                      {uidResult.candidate?.total_sessions} completed
                      interview(s)
                    </p>
                  </div>
                </div>

                <h4 style={{ margin: "20px 0 12px", color: "#94a3b8" }}>
                  Interview Sessions
                </h4>
                  ...
                  {uidResult.sessions?.map((s) => (
                    ...
                    >
                      <div className="hr-result-card-top">
                        <div className="hr-result-role">
                          {s.role || "General"}
                        </div>
                        <div
                          className="hr-result-score"
                      ...
                      <div className="hr-result-meta">
                        <span className={`hr-difficulty-badge ${s.difficulty}`}>
                          {s.difficulty}
                        </span>
                      ...
                      <div className="hr-result-view-btn">
                        View Full Details →
                      </div>
                    </div>
                  ))}
          ...
          <div className="hr-search-panel">
            ...
            {availableRoles.length === 0 && (
              <div className="hr-empty-state">
                <p>
                  No roles available yet. Candidates haven't completed
            ...
            {roleResults && (
              <div>
                <div className="hr-results-summary">
                  <span>
                    Found <strong>{roleResults.count}</strong> candidate(s) for{" "}
                    <strong>{roleResults.role}</strong>
                  </span>
                </div>
                      ...
                      >
                        <div className="hr-result-card-top">
                          <div className="hr-candidate-mini">
                            ...
                            <div>
                              <div className="hr-result-name">
                                {item.candidate?.name}
                              </div>
                              <div className="hr-uid-badge-sm">
                                {item.candidate?.uid}
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="hr-result-meta">
                          <span className="hr-result-role-tag">
                            {item.role}
                          </span>
                          <span
                            className={`hr-difficulty-badge ${item.difficulty}`}
                          >
                            {item.difficulty}
                          </span>
                        </div>
        ...
        {detailLoading && (
          <div className="hr-loading-overlay">
            <Loader message="Loading session details..." />
          </div>
        )}
      </div>
    </div>
